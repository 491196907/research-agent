...
**Python 3.10+** · **Streamlit** · **DeepSeek API** · **向量检索（embedding）** · **SQLite** · **42 个单元测试（每次 push 自动跑）**
...
| **使用记录 + 多人口令**（新） | 每人一个口令（进去后自动认出"你是谁"）；每次研究都记一条（谁 / 何时 / 什么主题 / 几步 / 成没成），可一键导出 **CSV** 用 Excel 打开 |
...
| 跑测试（42 个用例，不花钱） | `py -m pytest tests -q` |
...
├── tests/test_core.py   ← 42 个测试（检索 / 工具 / 抓网页 / 数据库 / 循环 / 报告 / 回调 / 口令）
├── screenshots/         ← 界面截图
├── 资料库/              ← 你的资料放这里
...
**42 个单元测试，全部不花钱、不联网**（网络和 API 都用 `monkeypatch` 桩掉了），所以 CI 里不需要配任何密钥。
...
> 每次 `push` 都会在 GitHub 上自动跑一遍这 42 个测试（见页面顶部的 tests 徽章）。
