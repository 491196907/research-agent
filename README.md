...
**Python 3.10+** · **Streamlit** · **DeepSeek API** · **向量检索（embedding）** · **SQLite** · **47 个单元测试（每次 push 自动跑）**
...
| **使用记录 + 多人口令**（新） | 每人一个口令（进去后自动认出"你是谁"）；每次研究都记一条（谁 / 何时（北京时间）/ 什么主题 / 几步 / 成没成），可一键导出 **CSV** 用 Excel 打开 |
...
| 跑测试（47 个用例，不花钱） | `py -m pytest tests -q` |
...
├── tests/test_core.py   ← 47 个测试（检索 / 工具 / 抓网页 / 数据库 / 循环 / 报告 / 口令 / 时间）
├── screenshots/         ← 界面截图
├── 资料库/              ← 你的资料放这里
...
| 使用记录里的时间比我的表慢 8 小时 | 云端容器的时间是 UTC。新版已改成**按北京时间**写入（`config.TZ_OFFSET_HOURS = 8`）；重启前写入的旧记录仍是 UTC |
| **主题和报告对不上** | 旧版做完研究是"取 `outputs/` 里最新的文件"当报告 —— 多个人同时用就会张冠李戴。新版由 `agent.run()` 直接把**这一次**的报告路径交给界面 |
...
| **④ 每人一个口令**（配了就能看出谁是谁） | Secrets 里写 `ACCESS_PASSWORDS = "yaoyi=我自己, abc123=小明, def456=小红"`，一个口令发给一个人 | 想知道"谁在研究什么"（见应用里的「📊 使用记录」页） |
...
用**方式 ④ 每人一个口令**配置好之后，打开应用的 **「📊 使用记录」** 页就能看到：

- **总研究次数**和**参与人数**
- **按访问者统计**：小明 3 次、小红 1 次……
- **明细**：每一次研究的时间 / 访问者 / 主题 / 走了几步 / 完成还是提前停止 / 生成了哪份报告
- **⬇️ 下载 CSV**：导出成表格文件，双击就能用 Excel 打开

> ⚠️ Streamlit 免费版的应用重启（或闲置一段时间）后，**本地数据库会重置**，使用记录也就没了 ——
> 所以看到有用的数据就点一下「下载 CSV」存到自己电脑上。
> 这也顺便回答了一个常见问题："为什么数据库里的东西过几天不见了？"

> 📄 **CSV 是什么**：一种用逗号分隔的纯文本表格（`.csv`），第一行是表头，一行就是一条记录。
> Excel / WPS / Numbers / Google Sheets 都能直接打开，也能用代码读。
> 本项目导出时用的是 `utf-8-sig` 编码 —— 它会在文件开头多写 3 个字节的 BOM 标记，
> 这样 Excel 打开中文才不会变成乱码（不带 BOM 的 UTF-8 中文在 Excel 里经常是"锟斤拷"）。
...
**47 个单元测试，全部不花钱、不联网**（网络和 API 都用 `monkeypatch` 桩掉了），所以 CI 里不需要配任何密钥。
...
### 数据库与使用记录（6 个）· `store.py`

| 测什么 | 为什么重要 |
|--------|------------|
| 存一条要点：条数 +1，能读回来 | 基本读写 |
| **数据真的持久化**（关掉再打开还在） | 这是选 SQLite 而不是内存缓存的理由 |
| 使用记录：写进去、读回来、**最新的排最前** | 界面的"使用记录"页直接依赖它 |
| 按访问者统计次数（小明 3 次、小红 1 次） | 这就是"谁用了、用了多少"的来源 |
| 访问者名称为空 → 记成"访客" | 没配口令时也不能崩 |
| 步数传了脏数据（`"五步"`）→ 返回 False，**不写坏数据库** | 界面传进来的值不可信，失败也不能影响研究本身 |

### 口令与访问者（5 个）· `config.py`

| 测什么 | 为什么重要 |
|--------|------------|
| 解析 `"口令=名字, 口令2=名字2"`（含空格、缺名字的情况） | 配置写错一个空格不该让整个应用崩 |
| 空值 / 没配 → 返回 `{}`，不报错 | 没配口令是最常见的状态 |
| 多人模式：口令对 → 返回对应名字；前后空格能容错；乱猜 → `None` | 认人准不准，直接决定使用记录有没有意义 |
| 单人模式（只有一个 `ACCESS_PASSWORD`）→ 名字统一记"访客" | 兼容旧配置 |
| 完全没配口令 → 不设门，谁都能进 | 本地开发不要被自己锁在外面 |
...
### 时间与报告路径（5 个）· `config.py` / `report.py` / `agent.py`

| 测什么 | 为什么重要 |
|--------|------------|
| `config.now()` 真的带 +8 时区偏移 | **云端容器的"本地时间"是 UTC** —— 直接用 `datetime.now()` 会比你的表慢 8 小时；这条测试把"时间对不对"钉死 |
| 使用记录写入的时间是北京时间（前缀 = 今天） | 记录里"什么时候研究的"要能直接看懂，不用自己换算 |
| 报告文件名和"生成时间"都用北京时间 | 文件名里的时间戳同样是给人看的 |
| `agent.run()` 把**这次**的报告路径交给调用方 | 界面的下载按钮靠它拿到正确的那一份（取巧用"最新文件"会张冠李戴） |
| `on_step` 的 `done` 事件里带报告路径 | 界面边跑边显示，最后一步才知道文件叫什么 |

> 每次 `push` 都会在 GitHub 上自动跑一遍这 47 个测试（见页面顶部的 tests 徽章）。
